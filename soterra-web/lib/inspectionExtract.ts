...
- The scope description itself — the "what I looked at" narrative ("recheck of ICA fail items, north and east elevations", "postline for ground floor apartments"). BUT read the scope/overview block to its END, because the outstanding items are often written straight into that prose rather than a tidy list, especially on a recheck once the earlier items are noted closed. Use the OUTCOME to read a to-do line there: on a PARTIAL PASS or FAIL, a line phrased as outstanding work — "X to install", "X to complete", "X to come", "QA to come", "still to check", "query X, engineer to confirm" — IS an open defect and MUST be extracted even though it sits under the scope/overview heading. On a clean PASS the same phrasing is only a routine next-step (saw cuts / control joints after a pour, curing, "provide the producer statement / QA documentation at CCC", "book the next inspection") — skip those. Skip descriptive "what was inspected" sentences and routine next-steps on a pass; never skip an outstanding defect just because of where it sits.
