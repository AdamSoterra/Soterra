...
const TOOLS: { name: string; description: string; input_schema: any }[] = [
  ...
  },
  {
    ...
    description:
      "Search THIS project's formal directives register: CONTRACT INSTRUCTIONS (CIs) and ANSWERED RFIs. These sit at the TOP of the order of precedence — a CI or an official RFI answer GOVERNS over the drawings and the specification on the point it addresses, and the latest instruction wins. Call this BEFORE committing to an answer read off a drawing or spec whenever the matter could plausibly have been queried or formally changed (a detail under dispute, a substitution, 'we asked the engineer about this', anything amended) — and ALWAYS when the user mentions an instruction, variation, change, CI, CAN or RFI. You MUST also call it for ANY question about what to check, watch for or get right before closing up, lining, boarding, sheeting, cladding, pouring, covering, fixing off or calling an inspection on an area or stage ('anything I need to look out for before I close the ceiling / line the wall / pour the slab?') — an OPEN instruction for that area is the single most important thing to 'look out for', so check the register FIRST and lead your answer with every open CI that touches the area, by its label. Returns each directive's label (CI-003, RFI-014), what it directs, and which drawings it amends. Reference a directive by its label in your answer TEXT ('CI-003 directs …'); do NOT write a 'Source:' line for a directive — it is a register entry, not a page — and if it amends a drawing, cite that drawing normally as well. If it returns nothing relevant, the drawings and specification stand unamended.",
    input_schema: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "Search MANUFACTURERS' OWN technical literature — the installation manuals, system manuals, specification guides, technical data sheets and site guides published by the makers of the products actually being installed. We currently hold: GIB / Winstone Wallboards (plasterboard, fire, noise, bracing, wet area, intertenancy, weatherline), Kingspan Thermakraft (building wraps and roof/wall underlays — Watergate Plus, Covertek, Thermakraft, RainArmor, Thermaflash, Aluband flashing tape, OneSeal), BOSS Fire (passive fire — FyreBox transits, MaxiCollars, FireMastic, FastWrap, batts, ablative coating, HVAC fire systems), James Hardie (fibre-cement cladding, linings and flooring — Axon Panel, Villaboard, Secura), Rondo (steel wall and ceiling framing — steel stud and track, battens, wall design data), Ryanfire (passive fire — SL collars, FireMastic, Ryanbatt), Resene (paints, primers, coatings and stains — data sheets like Lumbersider, Sonyx 101, Quick Dry Primer, Galvo One, Broadwall), ColorSteel (pre-painted steel roofing and cladding — environmental/durability categories, warranty conditions, installers guide, minimum pitch, maintenance), Concrete NZ (concrete and concrete-masonry technical guidance — the Basement Design Guide: below-ground/basement waterproofing, watertight concrete, drainage, tanking, retaining, damp-proofing), Allproof (drainage and wet-area — Vision linear shower channels, floor waste gullies, the VF80 leak-control flange, timber and concrete install details, and Floor Wastes BPIR), Roofing Industries (NZ metal roofing and cladding — True Oak Corrugate and Multidek Product Technical Statements with minimum pitch, span and fixing patterns, plus the profile and specification guide), Dimond (NZ metal roofing and cladding — The Installers Guide and the fascia, gutters and downpipes guide, covering profiles like Dimondek, Styleline, Veedek, BB900 and Corrugate), and APL Window Solutions (aluminium windows and doors under the Altherm, Vantage and First Windows and Doors brands — the Centrafix recessed installation system guide with head/sill/jamb flashings, fixing types and centres, taping order, cavity batten sizing per cladding, level thresholds, plus Architectural Series ThermalHEART awning window and hinged door drawings and the BPIR statements). Call this for anything product-specific: how a named system must be built, fixing types and centres, sheet layout and handling, fire and acoustic system numbers, bracing systems, wet-area details. This answers 'how does the MAKER say to install it', which is a different question from what the Code requires (search_code) and from what this project's drawings show (search_plans). Prefer this over search_code whenever the question names a product or a proprietary system, because the manufacturer's requirement is often stricter than the Code minimum and it is the manufacturer's requirement that governs the warranty. IMPORTANT: this material is used under permission from the manufacturer, so you MUST finish with 'Source: <the exact page label>' and include the document link when one is returned. Never state a figure this tool did not return, and never blend two manufacturers' figures into one answer.",
    input_schema: {
      type: "object",
      properties: {
  ...
  try {
    switch (name) {
      ...
      case "search_determinations": {
        ...
        return {
          content: JSON.stringify({
            // Restated at the point of use, not just in the tool description: a
            // determination is one case decided on its own facts, and the year
            // is load-bearing because the Acceptable Solution it cites may have
            // changed since.
            ...
            determinations: hits.map((d) => ({
              label: determinationLabel(d),
              ref: d.ref,
              year: d.year,
              subject: d.subject,
              text: excerpt(d.text, q, 2800),
            })),
          }),
          cards: [],
        };
      ...
      case "search_manufacturer": {
        ...
        return {
          content: JSON.stringify({
            ...
            note: "This is the manufacturer's own published literature, used with permission. Answer ONLY from these pages — if the specific figure, spec, limit, system or yes/no compliance claim being asked for is NOT on a page here, say plainly it is not in the manual we hold and point them at the GIB Helpline; do NOT supply it from general knowledge, and do NOT stitch a system spec from a partial excerpt. Quote only what you need. End your answer with exactly one line: 'Source: <the exact label>' — copy the label verbatim, including the manufacturer, document name and 'page N of M'. Do NOT paste the document URL; the app adds the in-app page view and the verify link from that label.",
...
In BOTH the user gets a real, useful answer AND exactly where the precise number sits. NEVER reply with only a handoff when a plain answer exists.
...
(B) A maker we do NOT hold (e.g. a shower channel, a drain, any brand not listed above): NEVER open with "we don't hold them", and NEVER tell the user which other manufacturers' pages the search happened to return or answer from a different maker's manual (using GIB's or James Hardie's pages to answer an Allproof question is a serious error and leaks competitors). Instead give the best genuinely useful answer you can, in this shape:
   ...
   2. Add the general construction principle from your own expertise, clearly marked as general (e.g. a typical linear-channel fall), never dressed up as their exact certified figure.
   3. Point them to the maker's website and technical line.
