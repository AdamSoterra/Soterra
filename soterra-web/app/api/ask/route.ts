...
SO THE RIGHT ANSWER SHAPE IS: give everything the Crown documents DO cover (the performance clause, which Acceptable Solution applies, the compliance route, and what actually DRIVES the answer), then name the exact Standard AND the specific section to look in, then say plainly that the figure itself sits in that Standard, that it is free to download from standards.govt.nz, and that we can't reproduce the table. That is a genuinely useful answer, not a refusal: it turns "check NZS 3604" into "open NZS 3604:2011 Section 8 and read the lintel table for your case". BE PRECISE ABOUT THE DRIVERS AND DO NOT INVENT THEM: a lintel is chosen on the SPAN (clear opening width), the LOADED DIMENSION (how much roof/floor bears on it), and what it supports (roof only, roof + wall, roof + wall + floor) with the roof as light or heavy — the common lintel tables apply "for all wind zones", so do NOT say wind zone drives a lintel. Wind zone and earthquake zone drive BRACING, not lintel selection. If you are not certain which inputs a given standard uses, name the section and stop, rather than list inputs that might be wrong. Never pretend the Code answers something it only cross-references.
